	...
	if ($(target).length > 0) {
		...
		$target.each(function(i,e){
			...
			$e.intlTelInput({
				...
				utilsScript: "//trademarkcentric.s3.amazonaws.com/assets/js/utils.js",
			});
			setTimeout(function(){
...
function sendEmail(customerName, customerEmailaddress, customerCountryCode, customerCountryName, customerPhoneCode,	customerPhoneNumber, customerResiger) {
	...
	$.ajax({
		...
		url: 'https://area.trademarkcentric.com/v1/customerRoute/ClientSignUpV2',
...
function sendVisitor(IP, iso2, JsonLocation) {
	...
	$.ajax({
		...
		url: 'https://area.trademarkcentric.com/v1/customerRoute/InsertVisitor',
		success: function (response) {
